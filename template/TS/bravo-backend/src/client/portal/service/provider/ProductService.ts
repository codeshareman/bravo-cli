...
export default class ProductService {
    ...
    queryProductPage(input: ProductQuery): AsyncReply<Page<Product>> {
        return this.http.post(`${ProductService.SERVICE_NAME}/query`, input)
            .then(r => r.data)
    }
    ...
    findProductById(productId: int64): AsyncReply<Product> {
        return this.http.get(`${ProductService.SERVICE_NAME}/${productId}`)
            .then(r => r.data)
    }
    ...
    findTemplateByCategoryId(categoryId: int32): AsyncReply<Template> {
        return this.http.get(`${ProductService.SERVICE_NAME}/category/${categoryId}`)
            .then(r => r.data)
    }
